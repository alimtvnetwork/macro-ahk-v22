...
> - Only the **bare repo name** is rewritten (e.g. `macro-ahk-v21` → `macro-ahk-v22`).
...
… (URL-safety) skipped 3 matches inside URLs:
  readme.md:42  https://github.com/alimtvnetwork/macro-ahk-v21
  …
...
|---|------|-------------------|
| 1 | Repo name appears as a substring of a longer token (e.g. `macro-ahk-v210` when renaming `macro-ahk-v21`) | Match must be **whole token**: the byte before the match must not be `[A-Za-z0-9_-]` AND the byte after must not be `[A-Za-z0-9_-]`. |
...
| 4 | Repo name in CHANGELOG.md heading like `## [v21]` | Only matches that contain the full slug (`macro-ahk-v21`) are touched. Bare `v21` headings are left alone. |
...
| ID | Criterion |
|----|-----------|
| AC-RR-001 | Running `scripts/repo-rename.sh` with no args in a repo whose remote is `…/macro-ahk-v22` shows a dry-run with `From: macro-ahk-v21` / `To: macro-ahk-v22`. |
...
| AC-RR-003 | URL strings (`https://github.com/org/macro-ahk-v21`, `git@github.com:org/macro-ahk-v21.git`) are reported as URL skips and never rewritten. |
| AC-RR-004 | A line containing both `macro-ahk-v21` (bare) and `https://github.com/org/macro-ahk-v21` is partially rewritten — only the bare token changes. |
| AC-RR-005 | `macro-ahk-v210` is **not** matched when renaming `macro-ahk-v21` (whole-token rule). |
...
| `--commit` | Stage and commit the rewrite as a single conventional commit (`chore: rename macro-ahk-v21 → macro-ahk-v22`). |
